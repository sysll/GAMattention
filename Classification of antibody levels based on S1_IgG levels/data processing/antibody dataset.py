...
#临床结局  严重程度（最终） S1_IgG  是否进入ICU  发病天数 这几个特征和水平相关性强，所以去除掉
df = df.drop(['发病日期', '入院时间', '出院/死亡时间', '检测日期', '临床结局 ', '严重程度（最终）', 'S1_IgG', '是否进入ICU', '发病天数'], axis=1)

# 去掉存在部分'S1_IgG'缺失的行
df = df.dropna(axis=0, subset=['N_IgG'], inplace=False)
...
#在这丢掉病人的ID是为了初始化
train_drop = train.drop(['N_IgG', '病人ID', '性别', '高血压(0=无，1=有)', '糖尿病(0=无，1=有)'], axis=1)
...
data_transform.insert(0, 'N_IgG', train['N_IgG'].tolist())
data_transform.insert(0, '病人ID', train['病人ID'].tolist())
#把小于602的数据删掉
data_transform = data_transform[data_transform['N_IgG'] >= 602]
# #把大于10000的用1，小于的用10000代替
data_transform['N_IgG'] = data_transform['N_IgG'].apply(lambda x: 0 if x <= 10000 else 1)

data_transform.to_excel(
    "D:\\Users\\ASUS\\Desktop\\论文\\基于N_lgG的阴阳分类\\数据处理\\processed train_data without division.xlsx",
    engine="openpyxl")
...
test_data = test.drop(['N_IgG', '病人ID', '性别', '高血压(0=无，1=有)', '糖尿病(0=无，1=有)'], axis=1)
...
data_transform.insert(0, 'N_IgG', test['N_IgG'].tolist())
...
data_transform.to_excel(
    "D:\\Users\\ASUS\\Desktop\\论文\\基于N_lgG的阴阳分类\\数据处理\\processed test_data without division.xlsx",
    engine="openpyxl")
