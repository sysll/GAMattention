...
from sklearn.metrics import roc_curve, roc_auc_score, precision_recall_curve, average_precision_score       #40周围效果较好
input_size = 1
...
def confusion(y_true, y_pred):
    ...
    sns.heatmap(cm_proportions, annot=True, fmt=".2f", cmap='Blues', vmin=0.35, vmax=0.6)

    # 设置坐标轴标签和标题
    plt.xlabel("Predicted Label")
    plt.ylabel("True Label")
    plt.title("Seq2seq_SA")
...
class Seq2Seq(nn.Module):
    ...
    def forward(self, x):
        ...
        return torch.sigmoid(outputs), attn_weights, encoder_outputs, hidden_output, con1
...
for i in range(1,2):
    ...
    for a in range(num):
        ...
        e = e + 1
    print(predict)
    predict = torch.where(predict > 0.5, torch.tensor(1), torch.tensor(0))
    # print(predict)
    acc = (predict == y_test).sum().item() / len(y_test)
    print(acc)
